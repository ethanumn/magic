...
class SCData:
    def __init__(self, data, data_type="sc-seq", metadata=None, data_prefix=""):
        ...
        if data.columns.nlevels == 1:
            cols = [
                np.array(["data" for i in range(data.shape[1])]),
    ...
    def data(self, item):
        ...
        cols = [
            np.array(["data" for i in range(item.shape[1])]),
            np.array(item.columns.values),
        ]
    ...
    def pca(self, item):
        if not (
            isinstance(item, pd.DataFrame) or item is None or isinstance(item, bool)
        ):
            print(type(item))
        ...
        if not isinstance(item, pd.DataFrame):
            ...
            item.columns = [
                np.array(["PCA" for i in range(item.shape[1])]),
                item.columns.values,
            ]
    ...
    def tsne(self, item):
        ...
        if not isinstance(item, pd.DataFrame):
            ...
            item.columns = [
                np.array(["tSNE" for i in range(item.shape[1])]),
                item.columns.values,
            ]
            self._data = pd.concat([self._data, item], axis=1)
            self._tsne = True
    ...
    def diffusion_eigenvectors(self, item):
        ...
        if not isinstance(item, pd.DataFrame):
            ...
            item.columns = [
                np.array(["diffusion_eigenvectors" for i in range(item.shape[1])]),
                item.columns.values,
            ]
            self._data = pd.concat([self._data, item], axis=1)
    ...
    def from_csv(
        ...
        cell_axis=0,
        delimiter=",",
        rows_after_header_to_skip=0,
        cols_after_header_to_skip=0,
        normalize=True,
    ):
    ...
    def from_10x(cls, data_dir, use_ensemble_id=True, normalize=True):
        ...
        dataMatrix = dataMatrix.iloc[dataMatrix.index[to_keep], :].astype(np.float32)

        # Remove empty genes
        print("Removing empty genes")
        gene_sums = dataMatrix.sum(axis=0)
        to_keep = np.where(gene_sums > 0)[0]
        dataMatrix = dataMatrix.iloc[:, to_keep].astype(np.float32)
    ...
    def from_10x_HDF5(cls, filename, genome, use_ensemble_id=True, normalize=True):
        ...
        with tables.open_file(filename, "r") as f:
            ...
            cell_sums = dataMatrix.sum(axis=1)
            to_keep = np.where(cell_sums > 0)[0]
            dataMatrix = dataMatrix.iloc[dataMatrix.index[to_keep], :].astype(
                np.float32
            )
    ...
    def filter_scseq_data(
        self,
        filter_cell_min=0,
        filter_cell_max=np.inf,
        filter_gene_nonzero=None,
        filter_gene_mols=None,
    ):

        sums = self.data.sum(axis=1)
        to_keep = np.intersect1d(
            np.where(sums >= filter_cell_min)[0], np.where(sums <= filter_cell_max)[0]
        )
        self.data = self.data.iloc[self.data.index[to_keep], :].astype(np.float32)

        if filter_gene_nonzero != None:
            nonzero = self.data.astype(bool).sum(axis=0)
            to_keep = np.where(nonzero >= filter_gene_nonzero)[0]
            self.data = self.data.iloc[:, to_keep].astype(np.float32)
    ...
    def normalize_scseq_data(self):
        ...
        scdata.data = scdata.data.iloc[:, nonzero_genes].astype(np.float32)
    ...
    def run_pca(self, n_components=100, random=True):
        ...
        self.pca = pd.DataFrame(
            ...
            columns=[
                self._data_prefix + "PC" + str(i) for i in range(1, n_components + 1)
            ],
        )

    def plot_pca_variance_explained(
        self, n_components=30, fig=None, ax=None, ylim=(0, 100), random=True
    ):
    ...
    def run_tsne(self, n_components=50, perplexity=30, n_iter=1000, theta=0.5):
        ...
        if data.shape[0] < 100 and perplexity > perplexity_limit:
            print(
                "Reducing perplexity to %d since there are <100 cells in the dataset. "
                % perplexity_limit
            )
        tsne = TSNE(
            n_components=2,
        ...
        self.tsne = pd.DataFrame(
            ...
            index=self.data.index,
            columns=[self._data_prefix + "tSNE" + str(i) for i in range(1, 3)],
        )

    def plot_tsne(
        self, fig=None, ax=None, density=False, color=None, title="tSNE projection"
    ):
    ...
    def plot_tsne_by_cell_sizes(self, fig=None, ax=None, vmin=None, vmax=None):
        ...
        plt.scatter(
            self.tsne["tSNE1"], self.tsne["tSNE2"], s=size, c=sizes, edgecolors="none"
        )
        plt.colorbar()
        plt.axis("tight")
        plt.tight_layout()
    ...
    def run_diffusion_map(
        ...
        if n_pca_components != None:
            if isinstance(self.pca, pd.DataFrame) and n_pca_components == len(
                self.pca.columns.values
            ):
                data = self.pca
            else:
        ...
        # Update object
        self.diffusion_eigenvectors = pd.DataFrame(
            V,
            index=self.data.index,
            columns=[
                self._data_prefix + "DC" + str(i) for i in range(n_diffusion_components)
            ],
        )
    ...
    def plot_diffusion_components(self, other_data=None, title="Diffusion Components"):
        ...
        for i in range(self.diffusion_eigenvectors.shape[1]):
            ...
            plt.scatter(
                self.tsne["tSNE1"],
                self.tsne["tSNE2"],
                c=self.diffusion_eigenvectors[i],
                edgecolors="none",
                s=size,
            )
        ...
        if other_data:
            for i in range(other_data.diffusion_eigenvectors.shape[1]):
                ax = plt.subplot(
                    gs[
                        (self.diffusion_eigenvectors.shape[1] + i) // n_cols,
                        (self.diffusion_eigenvectors.shape[1] + i) % n_cols,
                    ]
                )

                plt.scatter(
                    other_data.tsne["tSNE1"],
                    other_data.tsne["tSNE2"],
                    c=other_data.diffusion_eigenvectors[i],
                    edgecolors="none",
                    s=size,
                )
    ...
    def plot_diffusion_eigen_vectors(
        self, fig=None, ax=None, title="Diffusion eigen vectors"
    ):
    ...
    def run_diffusion_map_correlations(self, components=None, no_cells=10):
        ...
        for component_index in components:
            component_data = self.diffusion_eigenvectors.iloc[:, component_index]
            ...
            vals = self.data.iloc[order, :].rolling(no_cells).mean()[no_cells:].values
            # vals = pd.rolling_mean(self.data.iloc[order, :], no_cells, axis=0)[no_cells:]
    ...
    def plot_gene_expression(self, genes, other_data=None):
        ...
        if not isinstance(genes, dict):
            not_in_dataframe = set(genes).difference(
                self.extended_data.columns.get_level_values(1)
            )
            if not_in_dataframe:
                if len(not_in_dataframe) < len(genes):
                    print(
                        "The following genes were either not observed in the experiment, "
                        "or the wrong gene symbol was used: {!r}".format(
                            not_in_dataframe
                        )
                    )
                else:
        ...
        for i, g in enumerate(genes):
            if not isinstance(genes, dict):
                g = self.extended_data.columns.values[
                    np.where([g in col for col in self.extended_data.columns.values])[0]
                ][0]
            ax = plt.subplot(gs[i // n_cols, i % n_cols])
            axes.append(ax)
        ...
        if other_data:
            for i, g in enumerate(genes):
                if not isinstance(genes, dict):
                    g = self.extended_data.columns.values[
                        np.where(
                            [g in col for col in self.extended_data.columns.values]
                        )[0]
                    ][0]

                ax = plt.subplot(
                    gs[(n_rows * n_cols + i) // n_cols, (n_rows * n_cols + i) % n_cols]
                )
                axes.append(ax)
                if isinstance(genes, dict):
                    color = genes[g]
    ...
    def scatter_gene_expression(
        self, genes, density=False, color=None, fig=None, ax=None
    ):
        ...
        for i in range(len(genes)):
            genes[i] = self.extended_data.columns.values[
                np.where(
                    [genes[i] in col for col in self.extended_data.columns.values]
                )[0]
            ][0]
        ...
        if len(genes) == 2:
            if density == True:
                # Calculate the point density
                xy = np.vstack(
                    [self.extended_data[genes[0]], self.extended_data[genes[1]]]
                )
                z = gaussian_kde(xy)(xy)
                ...
                idx = z.argsort()
                x, y, z = (
                    self.extended_data[genes[0]][idx],
                    self.extended_data[genes[1]][idx],
                    z[idx],
                )
                ...
                plt.scatter(
                    self.extended_data[genes[0]],
                    self.extended_data[genes[1]],
                    s=size,
                    c=color,
                    edgecolors="none",
                )
                ...
                color = self.extended_data.columns.values[
                    np.where(
                        [color in col for col in self.extended_data.columns.values]
                    )[0]
                ][0]
                plt.scatter(
                    self.extended_data[genes[0]],
    ...
    def scatter_gene_expression_against_other_data(
        self, genes, other_data, density=False, color=None, fig=None, ax=None
    ):
        ...
        for i, g in enumerate(genes):
            ...
            if density == True:
                ...
                idx = z.argsort()
                x, y, z = (
                    self.extended_data[g][idx],
                    other_data.extended_data[g][idx],
                    z[idx],
                )
                ...
                plt.scatter(
                    self.extended_data[g],
                    other_data.extended_data[g],
                    s=size,
                    c=color,
                    edgecolors="none",
                )
    ...
    def run_magic(
        ...
        new_data = pd.DataFrame(
            new_data,
            index=self.data.index,
            columns=["MAGIC " + gene for gene in self.data.columns.values],
        )
